...
# Usage: ./run_naabu_scan.sh <target>
if [ -z "$1" ]; then
  echo "Usage: $0 <target>"
  exit 1
fi
...
PARSER_POD=$(kubectl get pods -n $NAMESPACE -o name | grep parse-naabu-scan | grep "$SCAN_NAME" | head -1 | sed 's|pod/||')
  ...
  --image=busybox --overrides='{
    "spec": {
      "volumes": [{
        ...
        "persistentVolumeClaim": {"claimName": "naabu-parser-pvc"}
      }],
      "containers": [{
        "name": "busybox",
        "image": "busybox",
        "command": ["sleep", "3600"],
        "volumeMounts": [{"name": "naabu-parser-pvc", "mountPath": "/mnt"}]
      }]
    }
  }' &
